...
(defparameter *emulate-stack-allocation-p* (progn #-(or sbcl ccl) 'on-demand))
  ...
  (if dimensionsp
      (with-gensyms (list)
        (let* ((dimensions (ensure-list dimensions))
          ...
          (when (eq *emulate-stack-allocation-p* 'on-demand)
...
(defun call-with-nested-stack (body)
  (let ((*emulate-stack-allocation-p* 'on-demand))
    (with-gensyms (result error-info)
